...
func (m restoreModel) View() string {
	...
	// resources
	if m.foundSummary && (m.fileCountTotal+m.symlinkCountTotal+m.xattrCountTotal) > 0 {
		done := m.countFilesOk + m.countFilesErrors + m.countSymlinksOk + m.countSymlinksErrors + m.countXattrsOk + m.countXattrsErrors
